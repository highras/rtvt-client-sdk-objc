...
inline static int mp3dec_load_index(void *user_data, const uint8_t *frame, int frame_size, int free_format_bytes, size_t buf_size, uint64_t offset, mp3dec_frame_info_t *info)
{
...
inline static size_t mp3dec_idx_binary_search(mp3dec_index_t *idx, uint64_t position)
{
...
inline static void mp3dec_close_file(mp3dec_map_info_t *map_info)
{
...
inline static int mp3dec_detect_mapinfo(mp3dec_map_info_t *map_info)
{
...
inline static int mp3dec_load_mapinfo(mp3dec_t *dec, mp3dec_map_info_t *map_info, mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb, void *user_data)
{
...
inline static int mp3dec_iterate_mapinfo(mp3dec_map_info_t *map_info, MP3D_ITERATE_CB callback, void *user_data)
{
...
inline static int mp3dec_ex_open_mapinfo(mp3dec_ex_t *dec, int flags)
{
...
inline int mp3dec_detect(const char *file_name)
{
    int ret;
...
inline int mp3dec_load(mp3dec_t *dec, const char *file_name, mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb, void *user_data)
{
    int ret;
    mp3dec_map_info_t map_info;
    if ((ret = mp3dec_open_file(file_name, &map_info)))
...
inline int mp3dec_iterate(const char *file_name, MP3D_ITERATE_CB callback, void *user_data)
{
    int ret;
    mp3dec_map_info_t map_info;
    if ((ret = mp3dec_open_file(file_name, &map_info)))
...
inline int mp3dec_ex_open(mp3dec_ex_t *dec, const char *file_name, int flags)
{
    int ret;
    if (!dec)
        return MP3D_E_PARAM;
